...
class DataValidation:
    ...
    def validate_number_of_columns(self,dataframe:pd.DataFrame) ->bool:
        try:
            number_of_colums = len(self.schema_config)
            logging.info(f"Required Number of Columns {number_of_colums}")
            logging.info(f"Data Frame has colums: {len(dataframe.columns)}")
            
            if len(dataframe.columns) == number_of_colums:
                return True
            return False
    ...
    def detect_dataset_drift(self,base_df,current_df,threshold=0.05) -> bool:
        try:
            ...
            drift_report_file_name = self.data_validation_config.drift_report_file_name
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            train_file_path = self.data_ingestion_artifact.trainded_file_path
